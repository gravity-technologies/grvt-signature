/**
 * This file holds the EIP712 types for the different actions that use for signing message.
 */

function keyMirror (originObj: Record<string, any>): Record<string, string> {
  return Object.entries(originObj).reduce<Record<string, string>>((obj, [key, value]) => {
    obj[key] = key
    return obj
  }, {})
}

const PrimaryType = keyMirror({
  // Account
  CreateAccount: 0,
